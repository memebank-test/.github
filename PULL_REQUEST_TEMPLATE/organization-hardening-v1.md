...
Describe the behavior under test, production repositories/components affected, compatibility impact, and rollback path. Mark non-applicable checks as `N/A` with a reason.

## Test-organization hardening gates

- [ ] This work is on a topic branch; no direct default-branch commit is required.
- [ ] Production source inputs are pinned by immutable commit, lockfile, or released Zed package.
...
- [ ] Unit, integration, adversarial, migration, clean-environment e2e, failure, recovery, and teardown evidence is attached.
